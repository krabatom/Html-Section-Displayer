...
object TextManager {
     ...
     */
    fun decorateText(text: String, tag: String): SpannableString {
    ...
    fun decorateText(text: String, tags: ArrayList<String>): SpannableString {
        val adjText = adjustText(text)
        if (!chars.contains(adjText)) {
            if (tags.size > 1){
                return decorateItalicAndBold(adjText)
            }
            else {
                for (tag in tags) {
                    when (tag){
...
//        if (!nText.isEmpty() || nText == " ") nText += " "
    ...
    private fun decorateItalicAndBold(text: String): SpannableString {
        val spText = SpannableString(text)
        spText.setSpan(StyleSpan(Typeface.BOLD_ITALIC), 0, text.count(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        return spText
    }
